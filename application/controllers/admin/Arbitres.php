...
class Arbitres extends CI_Controller {
    ...
    function designacions() {
        if ($this->session->userdata('logged_in')) {
            ...
            //echo "<pre>"; print_r($data); echo "</pre>";
            $this->load->view('admin/arbitres_designacions', $data);
            $this->load->view('templates/admin/footer');
        } else {
            redirect('admin/login', 'refresh');
        }
    ...
    function designa($idMatch) {
        if ($this->session->userdata('logged_in')) {
            $session = $this->session->userdata('logged_in');
            //print_r($_POST);
            $idReferee = $this->competicio_model->insert_referee_in_match($idMatch);
            $this->log_model->insert_log($session['id'], $idMatch, 'ha assignat un arbitre al partit');
          if($_POST['enviar']){  redirect("admin/arbitres/designacions");}
        } else {
            redirect('admin/login', 'refresh');
        }
    }
