...
//If no session, redirect to login page
    ...
    function introduir_equip() {
        if ($this->session->userdata('logged_in')) {
            ...
            $this->ordenar_posicions($idCompetition);
    ...
    function ordenar_posicions($idLeague) {
...
// echo "JORNADA: " . $match->idRound . " LOCAL: " . $match->idLocal . " VISITANT: " . $match->idVisitor . "<br />";
    ...
    function generar_calendari_eliminatoria($idLeague, $numberOfPhases) {
        if ($this->session->userdata('logged_in')) {
            $this->ordenar_posicions($idLeague);
            $session = $this->session->userdata('logged_in');
            $this->competicio_model->clear_calendar($idLeague);
            ...
            for ($a = $rounds; $a >= 1; $a--) {
                ...
                $idRound = $this->competicio_model->insert_round($r, $idLeague);
                echo " <hr />ID ROUND: " . $idRound;
                for ($b = 1; $b <= $numberOfMatches; $b++) {
                    echo "<br />$idRound $b";
                    $this->competicio_model->insert_matches_without_assignation($idRound, $idLeague, null, null);
                }
            }
            redirect('admin/competicio/calendari_copa/' . $idLeague, 'refresh');
        } else {
            redirect('admin/login', 'refresh');
        }
    }
    ...
    function calendari($idLeague) {
        if ($this->session->userdata('logged_in')) {
            ...
            $this->load->view('templates/admin/content');
            $this->load->view('admin/competicio_calendari', $data);
            $this->load->view('templates/admin/footer');
        } else {
//If no session, redirect to login page
            redirect('admin/login', 'refresh');
    ...
    function eliminatoria($idLeague) {
        if ($this->session->userdata('logged_in')) {
            $session = $this->session->userdata('logged_in');
            $this->load->view('templates/admin/header', $session);
            $data['get_mail_notread'] = $this->mail_model->get_mail_notread($session['id']);
            ...
            $data['get_all_status'] = $this->competicio_model->get_all_status();
            $this->load->view('templates/admin/content');
            $this->load->view('admin/competicio_eliminatoria_2', $data);
            $this->load->view('templates/admin/footer');
        } else {
//If no session, redirect to login page
            redirect('admin/login', 'refresh');
     ...
     function acta($idMatch) {
        if ($this->session->userdata('logged_in')) {
           ...
           // $data['get_all_complex'] = $this->competicio_model->get_all_complex();
            ...
            $data['localPlayers'] = $this->competicio_model->get_players_by_idTeam_and_idMatch($data['idLocal'], $idMatch);
            //echo "<pre>"; print_r( $data['localPlayers']); echo "</pre>";
            $data['visitorPlayers'] = $this->competicio_model->get_players_by_idTeam_and_idMatch($data['idVisitor'], $idMatch);

            //$data['referees'] = $this->competicio_model->get_referees();
            $data['referees_by_match'] = $this->competicio_model->get_referees_by_match($idMatch);

//echo "<pre>"; print_r($data); echo "</pre>";
            $this->load->view('admin/competicio_acta', $data);
            $this->load->view('templates/admin/footer');
        } else {
//If no session, redirect to login page
            redirect('admin/login', 'refresh');
...
//If no session, redirect to login page
    ...
    function modifica_equip_a_partit() {

        if ($this->session->userdata('logged_in')) {
            $session = $this->session->userdata('logged_in');
            $data['get_user_permissions'] = $this->user_model->get_user_permissions($session['id']);
            $data = $this->competicio_model->update_match_team();
            //$this->log_model->insert_log($session['id'], $idMatch, 'ha modificat un equip al partit ');
            //redirect('admin/competicio/partit/' . $idMatch, 'refresh');
        }
    }
...
//If no session, redirect to login page
    ...
    function partits() {
        if ($this->session->userdata('logged_in')) {
            ...
            $this->load->view('admin/menu', $data);
            $this->load->view('templates/admin/content');
            $data['get_next_week_matches'] = $this->competicio_model->match_queries();
            $data['get_all_complex'] = $this->competicio_model->get_all_complex();
            $data['get_all_teams'] = $this->competicio_model->get_all_teams();
            ...
            $data['get_all_matchstatus'] = $this->competicio_model->get_all_matchstatus();

            $this->load->view('admin/competicio_llista_partits', $data);
            $this->load->view('templates/admin/footer');
        } else {
//If no session, redirect to login page
            redirect('admin/login', 'refresh');
    ...
    function excel_partits() {

        // print_r($_POST);

        header('Content-Type: application/force-download');
        header('Content-disposition: attachment; filename=partits.xls');

        header("Pragma: ");
        header("Cache-Control: ");
        $data = $this->competicio_model->get_next_week_matches();
        //  print_r($data);           
        ...
        foreach ($data as $row):
            ...
            echo "<tr>";
            echo "<td>" . utf8_decode($row->local) . "</td>";
            echo "<td>" . utf8_decode($row->visitor) . "</td>";
            echo "<td>" . $date . "</td>";
            echo "<td>" . $hour . "</td>";
            echo "<td>" . utf8_decode($row->complexName) . "</td>";
            echo "<td>" . utf8_decode($row->league) . "</td>";
            echo "<td>" . utf8_decode($row->refereeString) . "</td>";
