...
class Control_economic extends CI_Controller {
    ...
    public function club($idClub) {
        if ($this->session->userdata('logged_in')) {
            ...
            $this->load->view('admin/menu', $data);
            $this->load->view('templates/admin/content');
            $data['get_economic_control_by_idClub'] = $this->Control_economic_model->get_economic_control_by_idClub($idClub);
            //echo "<pre>";print_r($data['get_economic_control_by_idClub']);echo "</pre>";
            $this->load->view('admin/control_economic_club', $data);
            $this->load->view('templates/admin/footer');
        } else {
            redirect('admin/login', 'refresh');
        }
    ...
    public function saldo($idClub) {
        ...
        // echo "<pre>"; print_r($data['get_payments']);echo "</pre>";
        foreach ($data['get_payments'] as $payment) {
            ...
            $type = 'Pagament';
            $this->Control_economic_model->insert_economic_control($idClub, $concept, $amount, $type, $datetime);
        }
        ...
        foreach ($data['get_refereed_matches'] as $match) {
            ...
            $concept = "Arbitratge " . $match->local . "-" . $match->visitor;
            $amount = $match->rate;
            $datetime = $match->updateddatetime;
            $type = 'Arbitratge';
            $this->Control_economic_model->insert_economic_control($idClub, $concept, $amount, $type, $datetime);
            //$table .= "\n<tr>\n\t<td>Arbitratge</td>\n\t<td>" . $match->local . "-" . $match->visitor . "</td>\n\t<td>" . $match->rate . "</td>\n\t<td> $balance</td>\n</tr>";
        }
        ...
        foreach ($data['get_cards'] as $card) {
            ...
            $type = 'Tarjetes';
            $this->Control_economic_model->insert_economic_control($idClub, $concept, $amount, $type, $datetime);
            //$table .= "\n<tr>\n\t<td>Tarjetes</td>\n\t<td>" . $card->name . " - " . $card->yellowCards . " tarjetes grogues i " . $card->blueCards . " tarjetes blaves</td>\n\t<td>$totalPrice</td>\n\t<td> $balance</td>\n</tr>";
        }
        //echo $table;
        $this->Control_economic_model->update_club_balance($idClub, $balance);
        // echo "<hr>Total: $balance";
        // exit();
    }
