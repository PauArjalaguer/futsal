...
<br />Banco Popular (Espanya) IBAN: ES11 0075 0973 0606 0609 4088', 'idSender' => 267, 'idReceiver' => $session['id']);
    ...
    function fer_ingres_admin($idClub = 0) {
        // $_POST['idClub']=$idClub;
        $idClub = $_POST['idClub'];
        if ($this->session->userdata('logged_in')) {
            $session = $this->session->userdata('logged_in');
            ...
            $this->saldo($idClub);
            //redirect("admin/control_economic/club/" . $idClub);
            redirect("admin/control_economic/admin");
        } else {
            redirect('admin/login', 'refresh');
        }
    }

    function introduir_moviment_admin() {
        // print_r($_POST);
        $idClub = $_POST['idClub'];
        if ($this->session->userdata('logged_in')) {
            $session = $this->session->userdata('logged_in');
            $data = $this->Control_economic_model->insert_movement();


            $this->log_model->insert_log($session['id'], $_POST['idClub'], 'ha fet un moviment al club.');
            $this->saldo($idClub);
            //redirect("admin/control_economic/club/" . $idClub);
            redirect("admin/control_economic/admin");
        } else {
    ...
    function excel_club($idClub) {

        $d = $this->clubs_model->get_club_info($idClub);
        $name = teamUrlFormat($d['name']);
        header('Content-Type: application/force-download');
        header('Content-disposition: attachment; filename=Extracte_economic_' . $name . '.xls');

        header("Pragma: ");
        header("Cache-Control: ");


        $get_economic_control_by_idClub = $this->Control_economic_model->get_economic_control_by_idClub($idClub);
        $get_payments = $this->Control_economic_model->get_payments($idClub);
//echo "<pre>"; print_r($get_economic_control_by_idClub);
        echo "<table border=0>";
        echo "<tr><td>Moviments</td></tr>";
        echo "</table>";
        echo "<table border=1>";
        echo "<tr><th>Data</th><th>Tipus</th><th>Concepte</th><th>Quantitat</th><th>Categoria</th><th>Pagat</th></tr>";
        foreach ($get_economic_control_by_idClub as $i) {
            echo "\n\t<tr>";
            echo "\n\t\t<td>" . invertdateformatshort($i->datetime) . "</td>";
            echo "\n\t\t<td>" . utf8_decode($i->type) . "</td>";
            echo "\n\t\t<td width=800>" . utf8_decode($i->concept) . "</td>";
            echo "\n\t\t<td>" . $i->amount . "</td>";
            echo "\n\t\t<td>" . utf8_decode($i->name) . "</td>";
             echo "\n\t\t<td>" . utf8_decode($i->isPaid) . "</td>";
            echo "\n\t</tr>";
        }
        echo "</table>";
        echo "<table border=0>";
        echo "<tr><td>Ingressos</td></tr>";
        echo "</table>";
        echo "<table border=1>";
        echo "<tr><th>Data</th><th>Quantitat</th></tr>";
        foreach ($get_payments as $i) {
            echo "\n\t<tr>";
            echo "\n\t\t<td>" . invertdateformatshort($i->datetime) . "</td>";
