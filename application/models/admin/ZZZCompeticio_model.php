...
where   r.idLeague=$idLeague"); //echo $this->db->last_query();
        return $query->result();
    }
...
join leagues l on l.id=r.idleague
join divisions d on d.id=l.iddivision
where l.idSeason=" . CURRENT_SEASON . " and (t1.idClub=$idClub or t2.idClub=$idClub ) and status=0 and updateddatetime!='0000-00-00 00:00:00' order by updateddatetime asc");
        return $query->result();
    }
...
//echo $this->db->last_query()."<br />";
    ...
    function get_next_week_matches() {
        $this->db->select('m.id as idMatch,t1.name as local, t2.name as visitor, updateddatetime, comment, r.name as roundName,l.name,c1.image as localImage, c2.image as visitorImage');
        $this->db->from('matches m');
        $this->db->join('teams t1', 'm.idLocal=t1.id');
        $this->db->join('teams t2', 'm.idVisitor=t2.id');
