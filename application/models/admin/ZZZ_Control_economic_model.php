...
//echo $this->db->last_query();
    ...
    function update_payment() {
        ...
        if ($_POST['date']) {
            $data = array('checked' => 1, 'datetime' => invertdateformat($_POST['date']));
        } else {
    ...
    function clubs_with_negative_balance() {
        $this->db->select('c.id,name,balance, totalPending,available,');
...
//echo "<hr />".$this->db->last_query();
    ...
    function get_economic_control_by_idClub($idClub) {
        ...
        $this->db->join('divisions d','d.id=aec.idDivision');
        $this->db->where('idClub', $idClub);
        $this->db->where('idSeason', CURRENT_SEASON);
    ...
    function update_economic_control_by_idClubAndType($idClub) {
        ...
        $q = $this->db->query("select COALESCE(SUM(amount),0) as amount from admEconomicControlPerClub where idClub=" . $idClub . " and type='Fitxa' and isPaid=1");
        $d = $q->row_array();
        // echo "<br />Total Jugadors pendents de pagar: " . 
        $totalPaidPlayers = $d['amount'];
        $totalPaid = $totalPaidPlayers;

        $q = $this->db->query("select COALESCE(SUM(amount),0) as amount from admEconomicControlPerClub where idClub=" . $idClub . " and type='Fitxa' and isPaid=0");
        $d = $q->row_array();
        // echo "<br />Total Jugadors pagats: " . 
        $totalPlayersToBePaid = $d['amount'];
        $totalPending = $totalPlayersToBePaid;
