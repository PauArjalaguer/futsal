...
$sql = "select m.id, datetime, datediff( datetime, now()) as dies, t1.name as local, t2.name as visitor, place, t1.idclub from matches m join teams t1 on t1.id=m.idlocal join teams t2 on t2.id=m.idvisitor where m.id=" . $_GET['idMatch'];
 //echo $sql;
