...
//Nom equip
//echo $_GET['idMatch'];
$res = mysql_query("SELECT t1.name as local, t2.name as visitor,localResult, visitorResult, idMatch, m.statusId, ms.color,t1.id as localId, t2.id as visitorId,ro.id as roundId,ro.name as roundName,l.id as idLeague,l.name as leagueName,m.comment ,t1.idclub as localClub, t2.idclub as visitorClub  from matches m
...
$row = mysql_fetch_array($res);
$localClub=$row['localClub'];
$visitorClub=$row['visitorClub'];
$localId=$row['localId'];
...
//Targetes
    ...
    $out .="<h2>" . $row['local'] . " <img onClick='matchSelectAllPlayersByIdTeamAndIdMatch(".$_GET['idMatch'].",".$localId.",".$localId.");' style='cursor:pointer;' src='images/user-plus.gif' /></h2>";
    ...
    $resPlayersL = mysql_query("
        ...
        and pts.idseason=$lastSeasonId and ispayed=1 and isdeleted!=1 order by pp.position,p.name") or die(mysql_error());
    $playersLocalArray = array();
 
    while ($rowPlayersL = mysql_fetch_array($resPlayersL)) {
		if($pos!=$rowPlayersL['position']){
			$out .="<br /><strong>".$rowPlayersL['position']."</strong><br />";
		}
        $checked = "";
        if ($n == 1) {
            $n = 2;
        ...
        if ($teamName != $rowPlayersL['teamName']) {
            $out .="<hr><div style='font-weight:bold;margin:10px 0;'>" . $rowPlayersL['teamName'] . "<img onClick='matchSelectAllPlayersByIdTeamAndIdMatch(".$_GET['idMatch'].",".$localId.",".$localId.");' style='cursor:pointer;' src='images/user-plus.gif' /></div>";
        }
    ...
    $resPlayersL = mysql_query("
        ...
        and pts.idseason=$lastSeasonId and ispayed=1 and isdeleted!=1 order by tcr.idTeamTransferer,pp.position,p.name") or die(mysql_error());
   ...
   $playersLocalArray = array();
    while ($rowPlayersL = mysql_fetch_array($resPlayersL)) {
        ...
        if ($teamName != $rowPlayersL['teamName']) {
            $out .="<hr><div style='font-weight:bold;margin:10px 0;'>" . $rowPlayersL['teamName'] . " <img onClick='matchSelectAllPlayersByIdTeamAndIdMatch(".$_GET['idMatch'].",".$rowPlayersL['teamId'].",".$localId.");' style='cursor:pointer;' src='images/user-plus.gif' /></div>";
        }
		if($pos!=$rowPlayersL['position']){
			$out .="<br /><strong>".$rowPlayersL['position']."</strong><br />";
		}
    ...
    $sql5="select id,name from teams where idclub=$localClub and id!=$localId and id not in (select idteamtransferer from teams_cession_relation where `idTeamTransfered`=$localId)";
    $res5=mysql_query($sql5) or die(mysql_error());
    $out .="<h2>Afegir equip.</h2>";
    $out .="<select id='matchTeamTransfererInsertSelect_$localClub' onChange='matchTeamTransfererInsert($localId, $localClub,".$_GET['idMatch'].")'><option></option>";
    while($row5=mysql_fetch_array($res5)){
      $out .="<option value=\"".$row5['id']."\">".$row5['name']."</option>";
    }
    $out .="</select>";
    $out .="</div>";
    $out .="<div style='width:50%; float:left;'>";
    $out .="<h2>" . $row['visitor'] . "<img onClick='matchSelectAllPlayersByIdTeamAndIdMatch(".$_GET['idMatch'].",".$visitorId.",".$visitorId.");' style='cursor:pointer;' src='images/user-plus.gif' /></h2>";
    ...
    $resPlayersV = mysql_query("
        ...
        order by pp.position,p.name") or die(mysql_error());
    $playersVisitorArray = array();
    while ($rowPlayersV = mysql_fetch_array($resPlayersV)) {
		if($pos!=$rowPlayersV['position']){
			$out .="<br /><strong>".$rowPlayersV['position']."</strong><br />";
		}
        $checked = "";
        if ($n == 1) {
            $n = 2;
    ...
    while ($rowPlayersV = mysql_fetch_array($resPlayersV)) {
        ...
        if ($teamName != $rowPlayersV['teamName']) {
            $out .="<hr><div style='font-weight:bold;margin:10px 0;'>" . $rowPlayersV['teamName'] . " <img onClick='matchSelectAllPlayersByIdTeamAndIdMatch(".$_GET['idMatch'].",".$rowPlayersV['teamId'].",".$visitorId.");' style='cursor:pointer;' src='images/user-plus.gif' /></div>";
        }
		if($pos!=$rowPlayersV['position']){
			$out .="<br /><strong>".$rowPlayersV['position']."</strong><br />";
		}
    ...
    $sql5="select id,name from teams where idclub=$visitorClub and id!=$visitorId and id not in (select idteamtransferer from teams_cession_relation where `idTeamTransfered`=$visitorId)";
    $res5=mysql_query($sql5) or die(mysql_error());
    $out .="<h2>Afegir equip.</h2>";
    $out .="<select id='matchTeamTransfererInsertSelect_$visitorClub' onChange='matchTeamTransfererInsert($visitorId, $visitorClub,".$_GET['idMatch'].")'><option></option>";
    while($row5=mysql_fetch_array($res5)){
      $out .="<option value=\"".$row5['id']."\">".$row5['name']."</option>";
    }
    $out .="</select>";
