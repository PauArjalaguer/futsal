...
$sql = "select
        ...
        , (select count(1) from cmptMatch_Referee where idMatch=m.id) as refereesCount
		 , (select count(1) from cmptMatch_Referee where accepted=1 and  idMatch=m.id) as refereesCountAccepted
,l.name as league
...
order by l.id,updateddatetime asc,round, datetime asc --  limit 0,3300";
...
while ($row = mysql_fetch_array($res)) {
    ...
    if ($a <= 1000) {
        ...
        $out .="<td class='zebra$n' align=center>".$row['refereesCountAccepted']. "/" . $row['refereesCount'] . "</td>";
