...
function rfrMatchAssignation(id){alert(id);
    //alert("a");
    var container1=document.getElementById("rightContent");
    ajax = nuevoAjax();
    ajax.open("GET", "rfrMatchAssignation.php?idLeague="+id, true);
...
function rfrMatchAssignationsUpdateAllowanceByCmrId(idCmr, idMatch){
    maskContainer();

    var km=document.getElementById("rfrMatchAssignationsUpdateAllowanceByCmrId_"+idCmr).value;
    if(km){
        var container1=document.getElementById("rightContent");
    }else{
        km=0;
    ...
    ajax = nuevoAjax();
    ajax.open("GET", "rfrMatchAssignationsUpdateAllowanceByCmrId.php?idCmr="+idCmr+"&km="+km, true);
...
function rfrMatchAssignation(id){
    //alert("a");
    var container1=document.getElementById("rightContent");
    ajax = nuevoAjax();
    ajax.open("GET", "rfrMatchAssignation.php?idLeague="+id, true);
...
function rfrRefereeEditSave(idReferee) {
	...
	var bankAccount=document.getElementById("rfrRefereeBankAccount").value;
    ...
    ajax.send("name="+name+"&email="+email+"&dni="+DNI+"&city="+city+"&province="+province+"&password="+password+"&idReferee="+idReferee+"&bankAccount="+bankAccount);
}

function rfrRefereeResendAccount(idReferee) { //alert(idReferee);
    maskContainer();


    ajax = nuevoAjax();

    ajax.open ('POST', 'rfrRefereeResendAccount.php', true);
    ajax.onreadystatechange = function() {
    ...
    ajax.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
    ajax.send("idReferee="+idReferee);
}

function rfrRefereeEditDelete(idReferee) {//alert(idReferee);
    maskContainer();


    ajax = nuevoAjax();

    ajax.open ('POST', 'rfrRefereeEditDelete.php', true);
    ajax.onreadystatechange = function() {
...
function rfrMatchAssignationPast(){
    //alert("a");
    var container1=document.getElementById("rightContent");
    ajax = nuevoAjax();
    ajax.open("GET", "rfrMatchAssignationPast.php", true);
