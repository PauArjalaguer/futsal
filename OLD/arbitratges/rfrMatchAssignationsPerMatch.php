...
$sqlMatch=" select t1.name as localName, t2.name as visitorName, t1.id as idLocal, t2.id as idVisitor from matches m
 join teams t1 on t1.id=m.idlocal
 join teams t2 on t2.id=m.idvisitor
 where m.id=".$_GET['idMatch'];
$res=mysql_query($sqlMatch) or die(mysql_error());
...
$sql="select updateddatetime from matches where id=".$_GET['idMatch'];
$res=mysql_query($sql) or die(mysql_error());
$row=mysql_fetch_array($res);
$matchDate=$row['updateddatetime'];

$combo = array();
//$comboSQL = "Select id, name, province FROM rfrReferees order by province asc, name asc";
$comboSQL = "Select id, name, province, (select count(*) from cmptMatch_Referee cmr
join matches m on m.id=cmr.idmatch 
where date_format('".$matchDate."',\"%j %y\") = date_format(updateddatetime, \"%j %y\") and idreferee=r.id  )
       FROM rfrReferees r 
left join rfrRefereesRecusedByIdTeam rfr on rfr.idReferee=r.id and (rfr.idTeam=$idLocal or rfr.`idTeam`=$idVisitor)
where rfr.idTeam is null and isDeleted!=1 
...
$sql = "select m.id ,t1.name as local ,t2.name as visitor ,r.name as referee ,r.id as idReferee ,cmr.id as idCmr, cmr.idRefereeType,km,allowance, ro.idleague, rprds.price as rPrice, rpmds.price as mPrice
FROM matches m
...
$out .="<div class='contentBoxHeader'><h2 onClick='rfrMatchAssignationsPerMatch(" . $_GET['idMatch'] . ");'> $localName - $visitorName</h2></div>";
$out .="<div class='contentBoxContent'>";
$out .="<table class='playersTable' cellspacing=0><tr><th width=350>Arbitre</th><th>Funcio</th><th>Kilometratge</th><th>Dieta</th><th>Preu</th></tr>";
...
if (mysql_num_rows($res) >= 1) {
    while ($row = mysql_fetch_array($res)) {
        ...
        foreach ($combo as $referees) {
            ...
            $out .="\n\t\t\t<option $selected value=\"" . $referees[0] . "\" >" . $referees[10] . " " . $referees[1] . " (".$referees[3].")</option>";
            $province = $referees[2];
        }
        ...
        $out .="<option disable>__________</option>";
        $out .="</select></td>";
        ...
        $out .="<td class='zebra$n'><input style='width:40px; text-align:center;' type=\"text\" id=\"rfrMatchAssignationsUpdateAllowanceByCmrId_" . $row['idCmr'] . "\" value=\"" . $row['allowance'] . "\" onKeyUp='rfrMatchAssignationsUpdateAllowanceByCmrId(" . $row['idCmr'] . "," . $_GET['idMatch'] . ");'></td>";
       
        $out .="<td class='zebra$n'>" . $row['rPrice'] . "</td></tr>";
...
foreach ($combo as $referees) {
    ...
    $out .="\n\t\t\t<option value=\"" . $referees[0] . "\" >" . $referees[10] . " " . $referees[1] . " (" . $referees[3] . ")</option>";
    $province = $referees[2];
}
$out .="</select></td></tr>";
...
$out .="<tr><td colspan=3 class='zebra1'>Retornar</td><td class='zebra1'>$total <br /><span style='cursor:pointer;' onClick='rfrMatchAssignationsPerMatch(" . $_GET['idMatch'] . ");'><img src='http://www.futsal.cat/administration/images/refresh.png' /></span></td></tr>";
