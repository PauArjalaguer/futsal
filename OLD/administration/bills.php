...
$out .= "<div style='width:50%; float:left; text-align:center;'><h4>FEDERACI&Oacute; CATALANA DE FUTBOL SALA</h4><br />CIF: G17102823<br /> APROVADA I INSCRITA PER LA SECRETARIA GRAL. DE L'ESPORT<br />REGISTRE N&UgraveM. 4.604 - 5 MAIG 1986</div>";
$out .= "<div style='width:30%; float:left; text-align:center;'>Rambla Guipuscoa 23-25 5&egrave; D <br />08018 Barcelona <br />Telf.: 93 244 44 03<br/ >Fax: 93 247 34 83</div>";
...
$out .="<tr><td  class='noBorder zeropadding bold'>Nom:</td><td colspan=4 class='noBorder zeropadding'>" . utf8_encode($name) . "</td></tr>";
$out .="<tr><td  class='noBorder zeropadding bold'>Domicili:</td><td colspan=4  class='noBorder zeropadding'>" . utf8_encode($address) . "," . utf8_encode($city) . "</td></tr>";
$out .="<tr><td  class='noBorder zeropadding bold'>Nif:</td><td colspan=4  class='noBorder zeropadding'>" . $row['nif'] . "</td></tr>";
$out .="<tr><td class='noBorder zeropadding'>&nbsp;</td></tr>";
$out .="<tr><td  class='noBorder zeropadding'>&nbsp;</td><td  class='noBorder zeropadding bold'>Num. Client:</td><td class='noBorder zeropadding bold'>DATA</td><td class='noBorder zeropadding bold'>Num. Factura</td><td class='noBorder zeropadding'>&nbsp;</td></tr>";
...
$out .="<tr><th width=5% >Quantitat</th><th width=50% colspan=4>Descripci&oacute;</th><th width=10% >P.Unitari</th><th width=10% >Import</th></tr>";
...
while ($row1 = mysql_fetch_array($res1)) {
    ...
    if ($r != 0) {
        ...
        $outa .= "Drets de competicio de " . $row1['name'] . " per a " . $row1['division'] . " t" . $seasonName;
        $out .= "Drets de competicio de " . $row1['prefix'] . " - t. " . $seasonName;
...
$res1 = mysql_query("select t1.name as local, t2.name as visitor, rate as price, updateddatetime from matches m
...
where statusId=4 and t1.idclub= $idClub  and updateddatetime>='$startDate' and updateddatetime<='$endDate'");
...
while ($row1 = mysql_fetch_array($res1)) {
    ...
    if ($n == 1) {
		...
		$concept=" Arbitratge ".utf8_encode($row1['local']). " - ".utf8_encode($row1['visitor']);
...
$out .="<tr style='display:none;'><td class='bold tdBottom' colspan=6>FORMA DE PAGAMENT</td><td class='bold tdRight tdBottom' colspan=6>IMPORTS</td></tr>";
...
where
    ...
    or  (cp.datetime>='$startDate' and cp.datetime<='$endDate' and cp.idPaymentType!=2))   and c.id=$idClub and archived=0");
$amount = 0;
while ($row1 = mysql_fetch_array($res1)) {
    ...
    $out .="<tr style='display:none;'><td class='noBorder tdLeft' style='border-left:1px solid #000; ' colspan=6>$d</td><td class='noBorder tdLeft tdRight' style='border-left:1px solid #000; border-right:1px solid #000;' colspan=6 align=right>" . $row1['amount'] . " &euro;</td></tr>";
    $amount = $amount + $row1['amount'];
}
$out .="<tr style='display:none;'><td class='noBorder tdLeft' style='border-bottom:1px solid #000;border-left:1px solid #000; ' colspan=6>&nbsp;</td><td class=' bold tdLeft tdBottom' style='border-left:1px solid #000; border-right:1px solid #000;' colspan=6 align=right>" . $amount . " &euro;</td></tr>";

$saldo = $amount - $total;
//echo "AMOUNT:$amount - TOTAL $total - SALDO $saldo";
if ($saldo > 0) {
    $out .="<tr style='display:none;'><td class='noBorder tdLeft' style='background-color:#fdd;border-bottom:1px solid #000;border-left:1px solid #000; ' colspan=6>Saldo a favor del club. </td><td class=' bold tdLeft tdBottom' style='border-left:1px solid #000;background-color:#fdd; border-right:1px solid #000;' colspan=6 align=right>" . $saldo . " &euro;</td></tr>";
}
