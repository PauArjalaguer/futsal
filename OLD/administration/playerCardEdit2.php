...
$sql = "
SELECT distinct p.name as playerName, floor(DATEDIFF(now(), birthdate) / 365.25) as age ,t.name as teamName, rate, paymentdate, d.name as divisionName, minAge, maxAge, minYear, maxYear, year(birthdate) as year, position
...
$sql = "
  select
    ...
    ,ispayed
	,(select count(*) from players p1 join player_team_season pts1 on pts1.idplayer=p1.id where p1.dni=p.dni and idseason=6 and statusPercent>50) as dniCount

  from players p
    join player_team_season pts on p.id=pts.idplayer
    left join playerCards_warned pcw on pcw.idCard=pts.id
  where p.id=" . $_GET['idPlayer'] . " and pts.idseason=$lastSeasonId";
//echo $sql;
