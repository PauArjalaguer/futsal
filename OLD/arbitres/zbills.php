...
//dades de facturació
$sql="SELECT r.name as round, datetime,d.name as division, t1.name as local, t2.name as visitor, cx.complexName, cx.complexAddress, cbi.name as billing_name, nif, address, city
FROM `matches` m
...
$out .="<tr><td  class='noBorder zeropadding bold'>Nom:</td><td colspan=4 class='noBorder zeropadding'>" . $row['billing_name'] . "</td></tr>";
$out .="<tr><td  class='noBorder zeropadding bold'>Domicili:</td><td colspan=4  class='noBorder zeropadding'>" . $row['address'] . "," . $row['city'] . "</td></tr>";
...
$out .="<tr><td  class='noBorder zeropadding'>&nbsp;</td><td class='noBorder zeropadding'>" . $row['round'] . "</td><td class='noBorder zeropadding'>" . invertdateformat($row['datetime']) . " $hour</td><td class='noBorder zeropadding'>".$row['division']."</td><td class='noBorder zeropadding'>&nbsp;</td></tr>";
...
$sql="select r.name as referee,m.id ,t1.name as local ,t2.name as visitor ,r.name as referee ,r.id as idReferee ,cmr.id as idCmr, cmr.idRefereeType,km, ro.idleague, rprds.price as rPrice, rpmds.price as mPrice, rt.refereeTypeName
FROM matches m
...
$out .="<tr><td class='zebra$n' align=center>&nbsp;</td><td class='zebra$n' nowrap colspan=5>Kilometratge</td><td class='tdRight' align=right>$km &euro;</td></tr>";
$out .="<tr><td class='tdBottom' align=center>&nbsp;</td><td class='tdBottom' nowrap colspan=5 class='tdBottom'>Preu arbitratge ".$row['division']."</td><td class='tdRight ' align=right>$matchPrice &euro;</td></tr>";
...
$out .="<tr><td colspan=5 class=' noBorder tdTop' style='border-left:0;'>&nbsp;</td><td class=' tdBottom' style='border-top:0;'  align=right>$msg</td><td class='tdRight tdBottom'  style='font-weight:bold;' align=right>$total &euro;</td></tr>";
