...
$sql = "select m.id,t1.name as local, t2.name as visitor,km, m.datetime, m.updateddatetime, rprds.price, rpmds.price as mPrice, prefix, ro.name as round from cmptMatch_Referee cmr
...
join rfrPricePerMatchbyDivisionAndSeason rpmds on rpmds.iddivision=l.iddivision and rpmds.idseason=$lastSeasonId
join rfrPricePerRefereeByDivisionAndSeason rprds on rprds.idseason=rpmds.idseason and rprds.iddivision=l.iddivision and rprds.idRefereeType=cmr.idRefereeType
 where idReferee=" . $_COOKIE['idReferee'];
//fecho $sql;
...
WHERE m.id=" . $row['id'];
    ...
    $out .="<td class='zebra$n pointer' align=center '><a style='color:#999;' href='bills.php?idMatch=" . $row['id'] . "'>Nomenament</a><br /><a style='color:#999;' href='billsPerClub.php?idMatch=" . $row['id'] . "'>Rebut club</a></td>";
