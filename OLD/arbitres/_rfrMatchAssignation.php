...
$sql = "select m.id,t1.name as local, t2.name as visitor, t1.id as idLocal, t2.id as idVisitor,km, m.datetime, m.updateddatetime, rprds.price, rpmds.price as mPrice, prefix, ro.name as round from cmptMatch_Referee cmr
...
join rfrPricePerRefereeByDivisionAndSeason rprds on rprds.idseason=rpmds.idseason and rprds.iddivision=l.iddivision and rprds.idRefereeType=cmr.idRefereeType
 where ro.idseason=$lastSeasonId and m.datetime >=now() and idReferee=" . $_COOKIE['idReferee']." order by m.datetime asc";
//echo $sql;
...
WHERE m.id=" . $row['id'];
    ...
    $out .="<td class='zebra$n pointer' align=center ' nowrap><a style='color:#999;' target=_blank  href='bills.php?idMatch=" . $row['id'] . "'>Nomenament</a><br /><a target=_blank style='color:#999;' href='billsPerClub.php?idMatch=" . $row['id'] . "'>Rebut lliga</a><br /><a style='color:#999;'  target=_blank href='billsPerClub.php?idMatch=" . $row['id'] . "&idTeam=".$row['idLocal']."&t=local'>Rebut copa local</a><br /><a style='color:#999;' target=_blank href='billsPerClub.php?idMatch=" . $row['id'] . "&idTeam=".$row['idVisitor']."&t=visitor'> Rebut copa visitant</a></td>";
